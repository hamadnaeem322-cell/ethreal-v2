...
const PHASES = [
  {
    ...
    title: "Where Time",
    italic: "Stands Still",
    body: "A boundless meadow at the edge of memory. Waiting for a footstep that will never arrive.",
  },
  {
    eyebrow: "The Resonance",
    title: "Shadows",
    italic: "In the Light",
    body: "Armor does not rust—it remembers. Every shadow is a prayer, every silence holds your name.",
  },
  {
    eyebrow: "The Surrender",
    title: "Only",
    italic: "An Echo",
    body: "A single petal falls. Some stories are written only to be felt, never to be held.",
  },
];
...
export const HeroScroll = () => {
  ...
  // Phase 1 disappears earlier so the "knight laydown" transition is clean.
  // The spatial movements pause while text is fully visible, moving only as they fade out to feel grounded.
  const p1 = useTransform(progress, [0.00, 0.08, 0.16], [1, 1, 0]);
  const p2 = useTransform(progress, [0.42, 0.47, 0.58, 0.65], [0, 1, 1, 0]);
  const p3 = useTransform(progress, [0.64, 0.74, 1.0], [0, 1, 1]);

  const p1Y = useTransform(progress, [0.0, 0.08, 0.16], ["0%", "0%", "-3%"]);
  const p2Y = useTransform(progress, [0.42, 0.47, 0.58, 0.65], ["2%", "0%", "0%", "-2%"]);
  const p3Y = useTransform(progress, [0.64, 0.74, 1.0], ["3%", "0%", "0%"]);

  // Set exceptionally subtle scale/blur for outgoing/incoming text
  const p1Scale = useTransform(progress, [0.08, 0.16], [1, 0.98]);
  const p2Scale = useTransform(progress, [0.42, 0.47, 0.58, 0.65], [0.99, 1, 1, 0.99]);
  const p3Scale = useTransform(progress, [0.64, 0.74], [0.98, 1]);
  
  const p1Filter = useTransform(progress, [0.08, 0.16], ["blur(0px)", "blur(8px)"]);
  // Phase 2 has a very delicate blur since the footage itself is a closeup with heavy depth-of-field
  const p2Filter = useTransform(progress, [0.42, 0.47, 0.58, 0.65], [3, 0, 0, 3]);
  const p2FilterStr = useTransform(p2Filter, (v) => `blur(${v}px)`);
  const p3FilterIn = useTransform(progress, [0.64, 0.74], ["blur(8px)", "blur(0px)"]);
        ...
        <div className="absolute inset-0 z-[10] flex items-center justify-start px-6 md:px-20 lg:px-28 pointer-events-none">
          <div className="relative w-full max-w-3xl h-[60vh] md:h-[55vh]">
            ...
            <motion.div
              ...
              className="absolute inset-0 flex flex-col justify-center pointer-events-auto will-change-transform"
            >
              <PhaseBlock data={PHASES[2]} index="03" />
            </motion.div>
          </div>
        </div>
...
const PhaseBlock = ({ data, index }) => {
  return (
    <div className="text-[#f2ebe5] w-full">
      ...
      <h1 className="font-display text-4xl md:text-5xl lg:text-[4rem] leading-[1.1] tracking-tight">
        {data.title}
        <br />
        <span className="italic text-[#e29578] opacity-90">{data.italic}</span>
      </h1>
      <p className="mt-4 md:mt-6 font-body text-sm md:text-base text-[#f2ebe5]/60 max-w-sm leading-relaxed font-light">
        {data.body}
      </p>
    </div>
  );
